...
import { ArrowRight, Check, ChevronRight, LoaderCircle, LockKeyhole, MessageCircle, PartyPopper, RotateCcw, ShieldCheck, ShoppingBag, Sparkles } from "lucide-react";
...
export default function Shopper() {
  ...
  const questionTotal = session?.profile.category === "headphones" || session?.profile.category === "running-shoes" ? 9 : session?.profile.category === "phones" ? 8 : 5;
  const answeredCount = Math.min(questionTotal, session?.profile.confirmedKeys.length ?? 0);
  ...
  return <main className="shop-shell"><header className="shop-header"><Link className="wordmark" href="/">choosy</Link><nav><Link href="/">Shop</Link><a href="#how">How it works</a><Link href="/agent-buyer">AI purchase demo</Link><Link className="merchant-link" href="/merchant"><LockKeyhole size={14}/> Merchant dashboard</Link></nav></header>
